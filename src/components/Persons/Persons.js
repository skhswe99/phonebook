...
const Persons = ({persons, filter}) => {
    return (
        <>
            ...
            <ul>
                {persons.filter(person => person.name.toLowerCase().indexOf(filter.toLowerCase()) >= 0 && /^[1-zA-Z]+$/.test(filter)).map(person => <li key={person.name} class='phone-entry'>{person.name} {person.phone}</li>)}
            </ul>
